...
export default function EditProductPage() {
  ...
  const [nameEwe, setNameEwe] = useState(product?.name.ewe ?? '');
  const [descFr, setDescFr] = useState(product?.description?.fr ?? '');
  const [descEn, setDescEn] = useState(product?.description?.en ?? '');
  const [descEwe, setDescEwe] = useState(product?.description?.ewe ?? '');
  const [slug, setSlug] = useState(product?.slug ?? '');
  const [categoryId, setCategoryId] = useState(product?.category?.id ?? '');
  const [priceXof, setPriceXof] = useState(product?.priceXof?.toString() ?? '');
  ...
  if (!product) {
    return (
      <div className="text-center py-20">
        <h1 className="text-2xl font-bold text-charcoal mb-2">Produit introuvable</h1>
        <p className="text-charcoal/60 mb-6">
          Aucun produit ne correspond a l&apos;identifiant &quot;{productId}&quot;.
        </p>
        <Link
          href="/admin/products"
          className="inline-flex items-center rounded-lg bg-gold px-4 py-2 text-sm font-semibold text-white hover:bg-gold/90 transition-colors"
        >
          ← Retour aux produits
        </Link>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-sm text-charcoal/70">Modifier</span>
      </div>
